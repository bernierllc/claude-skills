...
- Resolve the repo's actual default branch to a bare local name — `git symbolic-ref
  --short refs/remotes/origin/HEAD | sed 's|^origin/||'` (fallback: `gh repo view
  --json defaultBranchRef --jq .defaultBranchRef.name`) — never assume `main`/`master`,
  and never compare a remote-qualified name like `origin/main` against
  `git branch --show-current`. On that branch with anything to ship — uncommitted work
